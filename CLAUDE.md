...
**UI layer:** shadcn/ui style `aria-nova` (React Aria via `react-aria-components`), components pulled from the `@alexandremace` registry (ui.alexandremace.fr) into `components/ui/` — never edited locally, changes go through the kit. No `asChild`: links styled as buttons use the `LinkButton` export; interactions use `onPress`; selection styling via `data-selected`. Icons from `lucide-react` (1.x, no brand icons).
